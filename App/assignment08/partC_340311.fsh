...
{
    return vec3(texture(uTexDiffuse, vTexCoord));
}
...
vec3 specular() {
    ...
    vec3 specularMaterial = vec3(texture(uTexSpecular, vTexCoord)); // in part C this should be read from a second texture!
